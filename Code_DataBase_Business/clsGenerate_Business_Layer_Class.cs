...
namespace Code_DataBase_SibaSbahi_Business
{
    public class clsGenerate_Business_Layer_Class
    {
        ...
        {
            string ID = ""; StringBuilder functionBody = new StringBuilder();
            ...
            } // Remove the trailing comma and newline functionBody.Length -= 3;
            functionBody.AppendLine(" );"); functionBody.AppendLine($" return (this.{ID} != -1);");
            functionBody.AppendLine("}"); return functionBody.ToString();
        }
        ...
        private static string GenerateBusinessLayerUpdating(string className, List<clsColumn> ArrayOfColumns)
        {
            ...
            {
                functionBody.Append($" this.{column.ColumnName},");
            }
        ...
        private static string GenerateFindFunction(string className, List<clsColumn> ArrayOfColumns)
        {
            ...
            functionBody.AppendLine($"public static cls{className} Find(int {ObjectId})");
            functionBody.AppendLine("{");
            variables.Clear();
            refParameters.Clear();
            constructorParameters.Clear();
            for (int i = 0; i < ArrayOfColumns.Count; i++)
            {
                variables.AppendLine($"    {GetCSharpDataType(ArrayOfColumns[i].DataType)} {ArrayOfColumns[i].ColumnName} = {IfNullGetCSharpDataType(ArrayOfColumns[i].DataType)};");
                if (i == 0)
                {
                    refParameters.AppendLine($"    {ArrayOfColumns[i].ColumnName},");
                }
                else
                {
            ...
            functionBody.AppendLine(refParameters.ToString().TrimEnd(',', '\n'));
            functionBody.AppendLine("    );");
            functionBody.AppendLine("    if (IsFound)");
            functionBody.AppendLine("    {");
            functionBody.AppendLine($"        return new {className}(");
        ...
        private static string GenerateFindByNationalNumberFunction(string className, List<clsColumn> ArrayOfColumns)
        {
            StringBuilder functionBody = new StringBuilder();
            StringBuilder variables = new StringBuilder();
            StringBuilder refParameters = new StringBuilder();
            ...
            functionBody.AppendLine($"public static {className} Find(string NationalNumber)");
            functionBody.AppendLine("{");
            variables.Clear();
            refParameters.Clear();
            constructorParameters.Clear();
            ...
            functionBody.Append(variables.ToString());
            functionBody.AppendLine($"    bool IsFound = {className}_Data.Get{className}InfoBy(NationalNumber,");
            functionBody.AppendLine(refParameters.ToString().TrimEnd(',', '\n'));
            functionBody.AppendLine("    );");
            functionBody.AppendLine("    if (IsFound)");
            functionBody.AppendLine("    {");
        ...
        private static string GenerateFindByFullNameFunction(string className, List<clsColumn> ArrayOfColumns)
        {
            StringBuilder variables = new StringBuilder();
            StringBuilder functionBody = new StringBuilder();
            StringBuilder refParameters = new StringBuilder();
            StringBuilder constructorParameters = new StringBuilder();
            functionBody.Clear();
            functionBody.AppendLine($"public static {className} Find(string firstName, string lastName)");
            functionBody.AppendLine("{");
            variables.Clear();
            refParameters.Clear();
            constructorParameters.Clear();
            ...
            functionBody.Append(variables.ToString());
            functionBody.AppendLine($"    bool IsFound = {className}_Data.Get{className}InfoByFullName(firstName, lastName,");
            functionBody.AppendLine(refParameters.ToString().TrimEnd(',', '\n'));
            functionBody.AppendLine("    );");
            functionBody.AppendLine("    if (IsFound)");
            functionBody.AppendLine("    {");
        ...
        private static string GenerateIsObjectExistByNationalNoFunction(string ClassName)
        {
            ...
            functionBody.AppendLine("{");
            functionBody.AppendLine($"    return {ClassName}Data.Is{ClassName}FoundByNationalNumber(NationalNo);");
            functionBody.AppendLine("}");
            return functionBody.ToString();
        }

        private static string GenerateIsObjectExistByFullNameFunction(string ClassName)
        {
            StringBuilder functionBody = new StringBuilder();
            functionBody.AppendLine($"public static bool Is{ClassName}FoundByFullName(string firstName, string lastName)");
            functionBody.AppendLine("{");
            functionBody.AppendLine($"    return {ClassName}Data.Is{ClassName}FoundByFullName(firstName, lastName);");
            functionBody.AppendLine("}");
            return functionBody.ToString();
        }
        ...
        public static string GenerateAllBusinessLayerMethods(string ClassName, List<clsColumn> arrayOfColumns)
        {
            ...
            string findingNationalNumber = GenerateFindByNationalNumberFunction(ClassName, arrayOfColumns);
            ...
            fullCode.AppendLine(findingNationalNumber);
